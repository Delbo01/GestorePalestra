...
public class Calendario {
    ...
    void inserisciCorso(int mese,int giorno,Corso corso){
        calendario.get((mese - 1)).inserisciCorso(giorno,corso);
    }
    void rimuoviCorso(int mese,int giorno,Corso corso){
        calendario.get(mese - 1).rimuoviCorso(giorno,corso);
    }
    public void vediCorsiGiornalieri(int mese,int giorno){
        calendario.get(mese - 1).vediCorsiGiornalieri(giorno);
    }
    ...
    public boolean prenotaCorso(int mese,int giorno,Corso corso){
        return calendario.get(mese - 1).prenotaCorso(giorno,corso);
    }
    public boolean rimuoviPrenotazioneCorso(int mese,int giorno,Corso corso){
        return calendario.get(mese - 1).rimuoviPrenotazioneCorso(giorno,corso);
    }
